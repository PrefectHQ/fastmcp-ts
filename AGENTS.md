...
**Connection introspection:** `getServerInfo()` (the `serverInfo` Implementation: name, version, optional title/websiteUrl/icons), `getInstructions()`, and `getServerCapabilities()` surface the SDK client's post-connect state; all return `undefined` before `connect()`, and `getServerInfo()`/`getInstructions()` may be `undefined` on a modern-era connection (no `initialize` result).
