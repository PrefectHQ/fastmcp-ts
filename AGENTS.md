...
**Client identity:** `ClientOptions.clientInfo` and `MultiServerOptions.clientInfo` set the MCP `Implementation` advertised to servers. Both default to `{ name: 'fastmcp-ts', version: <installed package version> }`; the version is compiled from `package.json` during the build. Embedded applications should supply their own stable product identity.
