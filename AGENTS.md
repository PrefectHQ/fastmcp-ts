...
**Deliberate quirks (do not "fix" without a parity decision):** the flat input schema keeps an empty `required: []`; the `x-fastmcp-top-level-schema` marker leaks into advertised output schemas; the request-body content schema is mutated in place at parse time and the director reads the mutated version; a single body property is sent as the raw value (not JSON-wrapped) only when the content schema is a genuine primitive/array. All are Python behavior.
