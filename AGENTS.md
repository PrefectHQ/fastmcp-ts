...
**Client identity:** `ClientOptions.clientInfo` and `MultiServerOptions.clientInfo` set the MCP `Implementation` advertised to servers. Both default to `{ name: 'fastmcp-ts', version: <installed package version> }`; `package.json` is the version source. Embedded applications should supply their own stable product identity.
