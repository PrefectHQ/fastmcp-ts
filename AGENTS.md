...
**Client identity:** `ClientOptions.clientInfo` and `MultiServerOptions.clientInfo` set the MCP `Implementation` advertised to servers. Both default to `{ name: 'fastmcp-ts', version: '1.0.0' }`; embedded applications should supply their own stable product identity.
