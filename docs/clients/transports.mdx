...
A **string URL** speaks [Streamable HTTP](/servers/running#transports), the transport for servers exposed over the network. If the server only offers the older SSE endpoint, the client falls back to SSE on a 4xx response, so a URL works against both without special handling.
