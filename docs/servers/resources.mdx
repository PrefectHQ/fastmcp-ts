...
A resource's value can change after a client has read it, and subscriptions let the client find out instead of polling. The client subscribes to a URI and the server notifies it when that resource updates; the client then re-reads at its own pace. Subscription is a client-side action against a server that supports it, so the API lives on the [client](/clients/client).
...
The handler you pass fires whenever the server sends `notifications/resources/updated` for that URI. The notification carries only the URI, not the new value, which keeps updates cheap — the client reads the fresh data only when it's ready to use it.
