...
When your handler throws an ordinary error — anything that is not an `McpError` — FastMCP catches it and returns `{ isError: true }` with the message as content. The protocol call itself succeeds; the model receives the error as a tool result and can react to it, retry, or report it. This is the right channel for "the operation failed": a missing record, a downstream timeout, a refused request.

When the failure is protocol-level, throw an `McpError`. Input validation throws `McpError(InvalidParams)`; middleware such as rate limiting throws its own `McpError`; you can throw one directly when the client has violated the contract. An `McpError` propagates as a JSON-RPC error rather than a tool result, signalling that the request was malformed rather than that the work failed.
...
| `task` | Opt the tool into long-running [task](/servers/tasks) execution |
