...
| `MCP_HOST` | Bind host (HTTP) | Under review — set it explicitly |
...
The default bind host is an open decision, so pin the host you intend with `MCP_HOST` or the `host` run option rather than depend on the built-in fallback. The bind host also selects the DNS-rebinding posture below, which is why the two are decided together.
...
The bind host and this posture are coupled, which is why the default bind host is still an open decision: a routable default would leave a server open unless the operator opts in. Pin the host you intend, and set `dnsRebinding` to match the interface you bind.
