...
FastMCP clients reuse a fresh cached result by default for `listTools()`, `listPrompts()`, `listResources()`, `listResourceTemplates()`, and `readResource()`. Pass `cacheMode: 'refresh'` when you need a current result and want it to replace the cached entry. Pass `cacheMode: 'bypass'` for a one-off request that must neither read nor update the cache.

```typescript
import { Client } from '@prefecthq/fastmcp-ts/client'

const client = await Client.connect('http://localhost:3000/mcp')

const tools = await client.listTools({ cacheMode: 'refresh' })
const contents = await client.readResource('data:///report', {
  cacheMode: 'bypass',
})
