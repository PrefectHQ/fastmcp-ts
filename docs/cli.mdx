...
`install` points the host at a source file on this machine. To hand a server to people who do not have your checkout, publish it as an npm package that hosts launch with `npx`; [ship as a package](/servers/running#ship-as-a-package) covers the `bin` field and the shebang line that makes that work.
