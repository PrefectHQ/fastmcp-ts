...
The bind address itself is under review for 1.0. `MCP_HOST` sets the address an HTTP server binds to, and its default is being reconsidered alongside this security posture. Set `MCP_HOST` to the address you intend rather than relying on the default, and choose it deliberately — the host you bind and the rebinding guard are coupled. [Running a server](/servers/running) covers binding and the guard in full.
